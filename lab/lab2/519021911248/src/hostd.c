...
int main(int argc, char *argv[])
{
    ...
    FILE *inputliststream;                               // help to read dispatch file using FILE stream as common
    ...
    while (++i < argc)
    {
        // as the followings, set memory allocation algorihtm
        if (!strcmp(argv[i], "-mf"))
        {
    ...
    while (inputqueue || userjobqueue || (CheckQueues(dispatcherqueues) >= 0) ||
           currentprocess) // check the legal priority of process
    {
        ...
        while (inputqueue && inputqueue->arrivaltime <= timer) // the head-of-input-queue is ready to be dispatched
        {
            process = deqPcb(&inputqueue);        // dequeue process
            process->status = PCB_READY;          // set pcb ready
            if (process->priority == RT_PRIORITY) // if the process priority is (real-time = 0)
            {
                //           a. Real-time queue so check out parameters before
                //           enqueueing

                if (process->mbytes > RT_MEMORY_SIZE) // check if the resourse required is legal
                {
                    fprintf(stderr,
                            "\nERROR - overflow Real Time job memory max %d Mbytes - "
                            "job deleted\n",
                            RT_MEMORY_SIZE);
                    free(process); // abort the process
                    continue;
                }
                if (process->req.printers || process->req.scanners || process->req.modems ||
                    process->req.cds) // real time job are not allowed to use i/o
                {
                ...
                if (rtmemory) // allocate real-time job memory
                {
                ...
                else
                {
                    if (!memChk(&memory, process->mbytes))
                    {
                        fprintf(stderr, "\nERROR - Not enough memory for Real "
                                        "Time Job - job "
                                        "deleted\n"); // abort the process if memory is no longer enough
                        free(process);
                        continue;
                    }
            ...
            else if (process->priority > RT_PRIORITY && process->priority <= LOW_PRIORITY) // user job : 1 - 3
            {
                //               b. user job queue - check out parameters before
                //               enqueueing

                if (!memChkMax(process->mbytes)) // check if it is legal memory
                {
                    fprintf(stderr,
                ...
                if (!rsrcChkMax(process->req)) // check if it is legal resourse for i/o
                {
            ...
            else
            {
                ...
                free(process); // abort illegal process with other priorities
                continue;
            }
        }
        ...
        while (userjobqueue && memChk(&memory, userjobqueue->mbytes) &&
               rsrcChk(&resources, userjobqueue->req)) // need to check first
        {
            //           a. dequeue process from user job queue

            process = deqPcb(&userjobqueue); // dequeue head-of-userjobqueue

            //           b. allocate memory to the process

            process->memoryblock = memAlloc(&memory, process->mbytes); // allocate memory

            //           c. allocate i/o resources to process

            rsrcAlloc(&resources, process->req); // allocate i/o
            process->status = PCB_READY;         // set pcb ready
        ...
        if (currentprocess)
        {
            ...
            if (currentprocess->remainingcputime <=
                0) // the time is run out !! need to be redispatched, this time: TERMINATE
            {
            ...
            else if (currentprocess->priority > RT_PRIORITY &&
                     CheckQueues(dispatcherqueues) >=
                         0) // the time is run out !! need to be redispatched, this time: SUSPEND
            {
                ...
                if (++(currentprocess->priority) >
                    LOW_PRIORITY) // if currentprocess-priority is already 3, limit it up to 3
                    currentprocess->priority = LOW_PRIORITY;
                //++(currentprocess->priority), others just ++
                dispatcherqueues[currentprocess->priority] =
                    enqPcb(dispatcherqueues[currentprocess->priority], currentprocess); // reenqueue
                currentprocess = NULL;
            }
        }
        ...
        quantum = currentprocess && currentprocess->remainingcputime < QUANTUM ? currentprocess->remainingcputime
                  : !(currentprocess)                                          ? 1
                                                                               : QUANTUM;
        // if currentprocess is valid and its remainingcputime is less than 1, quantum = remainingcputime
        // otherwise, check if currentprocess is valid or not
        sleep(quantum);
