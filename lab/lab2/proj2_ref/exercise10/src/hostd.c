...
int main(int argc, char *argv[])
{
    ...
    while (inputqueue || userjobqueue || (CheckQueues(fbqueue) >= 0) || currentprocess)
    {
        ...
        while (inputqueue && inputqueue->arrivaltime <= timer)
        {
            ...
            userjobqueue = enqPcb(userjobqueue, process); // & put on queue
        }
        ...
        {
            process = deqPcb(&userjobqueue);                           // dequeue process
            process->memoryblock = memAlloc(&memory, process->mbytes); // alloc mam
            process->status = PCB_READY;                               // set pcb ready
            process->priority = 0;                                     // override any priority
